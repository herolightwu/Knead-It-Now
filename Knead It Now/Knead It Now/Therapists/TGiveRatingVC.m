...
@interface TGiveRatingVC ()
{
    NSString* reviewStr;
}
@end

@implementation TGiveRatingVC
...
- (void)viewDidLoad {
    ...
    imageLayer = self.mEditView.layer;
    [imageLayer setCornerRadius:1];
    [imageLayer setBorderWidth:1];
    [imageLayer setMasksToBounds:YES];
    [imageLayer setBorderColor:CONTROLL_EDGE_COLOR.CGColor];
...
- (void)setLayout{
    ...
    self.mRatingLabel.text = self.bookdata.buyer_rate;
...
- (IBAction)onPost:(id)sender {
    ...
    [HttpApi reportReview:self.bookdata.buyer_id FormId:g_user.userId Rate:rate BookId:self.bookid Content:reviewStr PostDate:postdate Success:^(NSDictionary *result){
