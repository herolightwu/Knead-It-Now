...
- (void) setAnnotation{
    if (self.mLocation!=nil &&![self.mLocation isEqualToString:@""]) {
        NSArray* foo = [self.mLocation componentsSeparatedByString:@","];
        g_latitude = foo[0];
        g_longitude = foo[1];
        ...
        if (![self.mAddress isEqualToString:@""]) {
            annotation.title = self.mAddress;
        }
    ...
    /*[[LMGeocoder sharedInstance] reverseGeocodeCoordinate:location
                                        ...
                                        completionHandler:^(NSArray *results, NSError *error) {
                                            if (results.count && !error) {
                                                ...
                                                self.mLocation = [NSString stringWithFormat:@"%@,%@", g_latitude, g_longitude];
                                                bUpdate = true;
                                            }
                                        }];*/
    ...
    [CLPlacemark hnk_placemarkFromGooglePlace: selectedPlace
                                   ...
                                   completion:^(CLPlacemark *placemark, NSString *addressString, NSError *error) {
                                       if (placemark) {
                                           ...
                                           self.mAddress = addressString;
                                           self.mLocation = [NSString stringWithFormat:@"%f,%f", placemark.location.coordinate.latitude, placemark.location.coordinate.longitude];
...
- (IBAction)onUpdateAddr:(id)sender {    
    if(bUpdate && [self.mLocation length] > 0 && [self.mAddress length] > 0){
        [self.delegate doneUpdateLocation:self CurLoc:self.mLocation CurAddr:self.mAddress];
        [self.navigationController popViewControllerAnimated:YES];
    }
}
@end
